...
def simulation_run(
        init_publisher_list, user_contexts, sigmoids, auction, num_iter, rounds_per_iter, soglia_ctr
):
    agent_stats = pd.DataFrame()
    comb_linucb = CUCB(publisher_list=init_publisher_list)
...
def run_simulation(output_dir, init_publisher_list, user_contexts, sigmoids, auction, num_iter, rounds_per_iter, soglia_ctr):
    print(f'Running simulation with soglia_ctr = {soglia_ctr}')
    budget_results = simulation_run(init_publisher_list, user_contexts, sigmoids, auction, num_iter, rounds_per_iter, soglia_ctr)
    agent_stats, lin_ucb_params = budget_results

    lin_ucb_params.to_csv(
        os.path.join(output_dir, f'agent_stats_{soglia_ctr}.csv'), index=False)
    ...
    group_iter['abs_err'] = np.abs(group_iter['ctr'] - group_iter['exp_rew'])
    group_iter['abs_perc_err'] = np.abs(group_iter['ctr'] - group_iter['exp_rew']) / group_iter['ctr']

    group_iter.to_csv(
        os.path.join(output_dir, f'grouped_results_per_iter_{soglia_ctr}.csv'),
        index=False)
...
if __name__ == "__main__":
    ...
    soglia_ctr_list = [0.8, 0.9, 0.95, 0.97, 0.99]
