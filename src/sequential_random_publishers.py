...
def run_experiment(run, rng, agent_configs, agents2item_values, agents2items, config, max_slots):
    ...
    auction, num_iter, _, output_dir = instantiate_auction(rng, config, agents2items, agents2item_values, agents,
                                                           max_slots)
    # Generare publishers casuali per ogni esperimento
    publisher_configs = get_random_publishers(100, 1000)
...
if __name__ == '__main__':
    ...
    for run in range(num_runs):
        run2agent2net_utility[run] = run_experiment(run, rng, agent_configs, agents2item_values, agents2items, config, max_slots)
