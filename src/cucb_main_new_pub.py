...
def simulation_run(
        run, init_publisher_list, sigmoids, new_pub_list, auction, num_iter, rounds_per_iter, soglia_ctr, alpha
):
    ...
    for i in range(num_iter):
        ...
        if i > 1:
            # Alla 250esima iterazione aggiungo i nuovi publisher senza 
            if i == 250:
                for pub in new_pub_list:
                    cucb.add_new_arm(pub)
...
def run_simulation(output_dir, run, init_publisher_list, new_pub_list, auction, num_iter, rounds_per_iter, soglia_ctr, alpha, embedding_size, adv_embeddings):
    ...
    budget_results = simulation_run(run, init_publisher_list, sigmoids, new_pub_list, auction, num_iter, rounds_per_iter, soglia_ctr, alpha)
...
if __name__ == "__main__":
    ...
    alpha_list = [1]
    soglia_ctr_list = [0.9, 0.95, 0.97]
    tasks = []
    for alpha in alpha_list:
        for soglia_ctr in soglia_ctr_list:
            for run in range(num_runs):
                tasks.append((output_dir, run, init_publisher_list, new_pub_list, auction, num_iter, rounds_per_iter, soglia_ctr, alpha, embedding_size, adv_embeddings))
