...
def run_iteration(auction, num_iter, run, output_dir, rounds_per_iter, lin_ucb, publishers):
    ...
    auction_revenue = []
    # Number of publishers for each deal
    k = 3
    with open(f'{output_dir}/agent_stats_{run}.csv', 'w') as f:
        f.write('Publisher,Won Auctions,Lost Auctions,Win Ratio,Num Clicks,Won True CTR,Lost True CTR,'
                'Won Estimated CTR,Lost Estimated CTR,Mean Won Bid,Mean Lost Bid,Mean Won Price,Mean Lost Price,Spent,Run,Iteration,Agent\n')
        for i in range(num_iter):
            ...
            for j in range(k):
                ...
                selected_publisher = lin_ucb.round_iteration_k(selected_publishers)
                # Remove the selected publisher from the list at the next iteration
                selected_publishers = [publisher for publisher in selected_publishers if publisher.name != selected_publisher]
                ...
                # Save the selected arm
                selected_arm['publisher'] = selected_publisher_object
                selected_arm['context'] = selected_publisher_object.embedding
                ...
                for agent in auction.agents:
                    if agent.name.startswith('Nostro'):
                        selected_arm['reward'] = agent.get_publisher_clicks(selected_publisher)

                arms_to_update.append(selected_arm)

            for agent in auction.agents:
                update_time = time.time()

                agent.update(iteration=i)
                # Save stats of our agent
                if agent.name.startswith('Nostro'):
            ...
            for arm in arms_to_update:
                lin_ucb.update(arm['context'], arm['publisher'], arm['reward'])
